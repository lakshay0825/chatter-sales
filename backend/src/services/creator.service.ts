...
export async function createCreator(input: CreateCreatorInput) {
  ...
  const creator = await prisma.creator.create({
    data: {
      ...
      revenueSharePercent: input.compensationType === 'PERCENTAGE' ? input.revenueSharePercent || null : null,
      fixedSalaryCost: input.compensationType === 'SALARY' ? input.fixedSalaryCost || null : null,
    },
  });
